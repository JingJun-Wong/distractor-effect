...
addpath(genpath([pwd,'/model-functions/'])) % add composite and prospect theory models
...
for s = 1:n_subj
    ...
    % PT:
    DN_flag = 0;
    PT.outputFull_B{s} = fitFunc_prospecttheory(attribute(tt==1,[1,2,4,5]),p_data(tt==1,:),Nfit,...
        distort_flag, DN_flag, parmEst);
    ...
    DN_flag = 0; % PT
    output = fitFunc_prospecttheory(attribute(trltrain,:),Y(trltrain,1:2),Nfit,distort_flag,DN_flag,[]);
    parmEst = output.Xfit; % get best parameters
    PT.testLL_B(s,1,fold) = fitFunc_prospecttheory(attribute(trltest,:),Y(trltest,1:2),Nfit,...
        distort_flag,DN_flag,parmEst);
...
save(['Figure3_Supplement2_Fits_',time_str,'.mat'],'AU','EV','EVDN','COMP','PT')
disp('done')
