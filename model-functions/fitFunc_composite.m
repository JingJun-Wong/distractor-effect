function output = fitFunc_composite(attribute, p_data, Nfit, distort_flag, DN_flag, parmEst)
...
if isempty(parmEst)
    ...
    B = [eta;beta;theta];
    B_lim = [eta_lim;beta_lim;theta_lim];
...
function [negLL, p_pred, rel_acc] = LL_func(attribute,p_data,DN_flag,eta,beta,theta)
...
% Utility:
compU = (eta * (att_X.*att_P)) + ((1-eta) * ((theta*att_X) + ((1-theta).*att_P)));
