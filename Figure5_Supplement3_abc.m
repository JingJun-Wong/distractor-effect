...
for s = multi_idx
    ...
    temp_hv = (int_coef(s)*(Xnorm(:,1).*Pnorm(:,1))) + ((1-int_coef(s)) * ((magprob_weight(s)*Xnorm(:,1))+((1-magprob_weight(s)).*Pnorm(:,1))));
    temp_lv = (int_coef(s)*(Xnorm(:,2).*Pnorm(:,2))) + ((1-int_coef(s)) * ((magprob_weight(s)*Xnorm(:,2))+((1-magprob_weight(s)).*Pnorm(:,2))));
    ind = find(temp_hv<temp_lv);
    temp_acc = data_acc;
    data_acc(ind) = 1-temp_acc(ind);
    ...
    val1 = (int_coef(s)*(regreMatrix(:,1).*regreMatrix(:,4))) + ((1-int_coef(s)) * ((magprob_weight(s)*regreMatrix(:,4))+((1-magprob_weight(s)).*regreMatrix(:,1))));
    val2 = (int_coef(s)*(regreMatrix(:,2).*regreMatrix(:,5))) + ((1-int_coef(s)) * ((magprob_weight(s)*regreMatrix(:,5))+((1-magprob_weight(s)).*regreMatrix(:,2))));
    hv = val1;
    lv = val2;
    ind = find(val1<val2);
    hv(ind) = val2(ind);
    lv(ind) = val1(ind);
    dv = (int_coef(s)*(regreMatrix(:,3).*regreMatrix(:,6))) + ((1-int_coef(s)) * ((magprob_weight(s)*regreMatrix(:,6))+((1-magprob_weight(s)).*regreMatrix(:,3))));
